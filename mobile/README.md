...
│   └── index.html      # HTML template (used by the local development server)
├── public/             # Static assets (manifest, icons)
│   └── manifest.json   # PWA manifest (used by the local development server)
├── app/                # Production build (committed to repo)
├── index.html          # Redirect to app/index.html (used by the local development server)
└── package.json        # Dependencies and scripts
```

Please note that the `index.html` and `manifest.json` files bundled with this project are only used by the local development server.
The real entry point and manifest are generated by the AI Commander plugin at the path specified in the plugin settings.
...
The app is also accessible directly from the plugin folder, at `wp-content/plugins/ai-commander/mobile`, for testing purposes, without the need to fire up a development server (so no `npm run dev` needed).
